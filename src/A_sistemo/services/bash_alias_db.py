...
_A_BASH_ALIASES_PERSONAL = Path.home() / ".A_bash_alias_personal"
...
class BashAliasDB:
    ...
    def sync_shell_config(self) -> None:
        """Sync aliases to shell config (A-managed, read-only)."""
        aliases = self.list_aliases()
        output = ["#!/bin/bash", "# Auto-generated by A (neeldonita), DO NOT EDIT", ""]
        for a in aliases:
            output.append(f'alias {a.alias}="{a.function}"')
        
        config = Path.home() / ".A_bash_alias"
        config.write_text("\n".join(output) + "\n")
        # Make read-only to prevent user edits
        config.chmod(0o444)
...
def migrate_bashrc(target_db: BashAliasDB) -> dict:
    ...
    for line in lines:
        ...
        if "autish_aliases" in line and "source" in line.lower():
            ...
            continue
        # Remove old A bash aliases source line
        if ".bash_aliases" in line and "source" in line.lower():
            continue
        # Skip if already have A source (new path)
    ...
    if not A_source_found:
        ...
        new_lines.append("source ~/.A_bash_alias_personal 2>/dev/null || true")
